...
Ideia → Base de Conhecimento → Estratégia → Framework → Copy → Revisão → Arte → Publicação → Atualização da Base
...
- **Claude Project:** instruções = doc 05; conhecimento = docs 00–04B + 06. Peça avulsa do dia a dia.
...
`/constituicao` (00–03) · `/conhecimento` (base, cases, estudos) · `/execucao` (04A, 04B, templates, prompts) · `/skills` · `/agents` · `/automacao` (n8n, notion, git) · `/outputs` · `/assets`.

## Regra de manutenção da Base

Todo ângulo publicado entra em "ganchos e ângulos já usados" do verbete. O gancho literal não se repete em intervalo curto; a tese se repete sempre, por novos ângulos.
